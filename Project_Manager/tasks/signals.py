    ...
    if created:
        if instance.parent_task:
            print(f'Команда - {instance.project.team.name} \n'
                  ...
                  f'{get_current_user()} создал подзадачу {instance.name} к задаче {instance.parent_task}'
                  )
        else:
            print(f'Команда - {instance.project.team.name} \n'
                  f'Проект - {instance.project.name} \n'
...
def change_task_signal(sender, instance, *args, **kwargs):
    ...
    if changes:
        if changes.get('name'):
            print(f'{get_current_user()} изменил имя задачи с {old_instance.name} на {instance.name}')
        if changes.get('description'):
            print(f'{get_current_user()} изменил описание задачи {instance.name} с {old_instance.description} на {instance.description}')
        if changes.get('status'):
            print(f'{get_current_user()} изменил статус задачи {instance.name} с {old_instance.status} на {instance.status}')
        if changes.get('priority'):
            print(f'{get_current_user()} изменил приоритет задачи {instance.name} с {old_instance.priority} на {instance.priority}')
...
def create_comment_tag(sender, instance, created, *args, **kwargs):
    if created:
        print(f'Задача - {instance.task.name} \n'
              f'{get_current_user()} оставил комментарий: {instance.content}'
        )
